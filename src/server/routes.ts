...
import { OpenAIChatRequest, ClaudeModel } from '../types/index.js';
...
async function handleStreamingResponse(
  ...
  try {
    ...
    logger.error('Streaming failed after retries', { error: (error as Error).message });
    if (!res.headersSent) {
      res.status(500).json({
        error: {
...
async function handleNonStreamingResponse(
  ...
  try {
    const result = await withRetry(async () => {
      ...
      return new Promise<{ text: string; toolCalls?: any[] }>((resolve, reject) => {
        let output = '';
        let errorOutput = '';
    ...
    logger.error('Request failed after retries', { error: (error as Error).message });
    res.status(500).json({
      error: {
        message: (error as Error).message,
        type: "server_error",
